<?php
/**
 * KE Lubricants SEO Booster - Diagnostic Script
 * 
...
<head>
    ...
    <title>KE Lubricants SEO Booster - Diagnostic Report</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            <h1>KE Lubricants SEO Booster - Diagnostic Report</h1>
        ...
        // Test 1: PHP Version
        echo '<div class="test ' . (version_compare(PHP_VERSION, '7.4', '>=') ? 'pass' : 'fail') . '">';
        echo '<h3>PHP Version Check</h3>';
        echo '<p>Current PHP Version: <strong>' . PHP_VERSION . '</strong></p>';
        echo '<p>Required: PHP 7.4 or higher</p>';
        
        if (version_compare(PHP_VERSION, '7.4', '<')) {
            $issues[] = 'PHP version ' . PHP_VERSION . ' is below the required 7.4';
            echo '<div class="details">This plugin requires PHP 7.4 or higher to function properly.</div>';
        } else {
        ...
        if (function_exists('get_bloginfo')) {
            ...
            echo '<div class="test ' . (version_compare($wp_version, '5.0', '>=') ? 'pass' : 'fail') . '">';
            echo '<h3>WordPress Version Check</h3>';
            echo '<p>Current WordPress Version: <strong>' . $wp_version . '</strong></p>';
            echo '<p>Required: WordPress 5.0 or higher</p>';
            
            if (version_compare($wp_version, '5.0', '<')) {
                $issues[] = 'WordPress version ' . $wp_version . ' is below the required 5.0';
                echo '<div class="details">This plugin requires WordPress 5.0 or higher to function properly.</div>';
            } else {
        ...
        echo '<div class="test ' . ($memory_limit_bytes >= 128 * 1024 * 1024 ? 'pass' : 'warning') . '">';
        echo '<h3>Memory Limit Check</h3>';
        echo '<p>Current Memory Limit: <strong>' . $memory_limit . '</strong></p>';
        echo '<p>Recommended: 128M or higher</p>';
        ...
        foreach ($writable_files as $file => $description) {
            $file_path = $plugin_dir . '/' . $file;
            if (file_exists($file_path)) {
                if (is_readable($file_path)) {
                    echo '<p>✓ ' . $file . ' - ' . $description . ' (readable)</p>';
                } else {
                    echo '<p>⚠ ' . $file . ' - ' . $description . ' (not readable)</p>';
                    $warnings[] = 'File not readable: ' . $file;
                }
            } else {
                echo '<p>⚠ ' . $file . ' - ' . $description . ' (does not exist)</p>';
                $warnings[] = 'File missing: ' . $file;
            }
        }
        echo '</div>';
        ...
        if (function_exists('get_option')) {
            ...
            foreach ($active_plugins as $plugin) {
                if (strpos($plugin, 'yoast') !== false || 
                    ...
                    strpos($plugin, 'kseo-seo-booster') !== false) {
        ...
        $wp_functions = array(
            ...
            'add_meta_box' => 'Meta box functionality',
            'wp_remote_post' => 'HTTP requests (for API calls)'
        );

        echo '<div class="test pass">';
        echo '<h3>WordPress Functions Check</h3>';
        
        foreach ($wp_functions as $function => $description) {
            if (function_exists($function)) {
                echo '<p>✓ ' . $function . ' - ' . $description . '</p>';
            } else {
                echo '<p>✗ ' . $function . ' - ' . $description . ' (NOT AVAILABLE)</p>';
                $issues[] = 'WordPress function not available: ' . $function;
            }
        }
        echo '</div>';
        ...
        if (empty($issues) && empty($warnings)) {
            echo '<p><strong>✓ All checks passed!</strong> Your system is ready to run KE Lubricants SEO Booster.</p>';
        } else {
            if (!empty($issues)) {
        ...
        echo '<li>Ensure you have a valid OpenAI API key</li>';
        echo '</ul>';
        echo '</div>';
        ?>
        ...
        <div class="recommendation">
            ...
            <ol>
                ...
                <li>Configure your OpenAI API key in the settings</li>
                <li>Test the AI generation functionality</li>
            </ol>
        </div>
        ...
        <div class="recommendation">
            ...
            <ul>
                <li><strong>White screen after activation:</strong> Check PHP error logs and ensure all files are present</li>
                <li><strong>AJAX not working:</strong> Verify nonce creation and WordPress AJAX setup</li>
                <li><strong>API calls failing:</strong> Check your OpenAI API key and internet connectivity</li>
                <li><strong>Meta boxes not appearing:</strong> Ensure post types are properly configured</li>
                <li><strong>JavaScript errors:</strong> Check browser console for conflicts with other plugins</li>
            </ul>
        </div>
    </div>
</body>
